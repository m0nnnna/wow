INSERT INTO `spell_dbc` SET `ID` = 22247, 
`Category` = 0, 
`DispelType` = 0, 
`Mechanic` = 0, 
`Attributes` = 0, 
...
`ImplicitTargetA_2` = 18, 
`ImplicitTargetA_3` = 18, 
`ImplicitTargetB_1` = 16, 
`ImplicitTargetB_2` = 16, 
`ImplicitTargetB_3` = 16, 
`EffectRadiusIndex_1` = 9, 
`EffectRadiusIndex_2` = 9, 
`EffectRadiusIndex_3` = 9, 
`EffectAura_1` = 33, 
`EffectAura_2` = 138, 
`EffectAura_3` = 65, 
...
`SpellVisualID_1` = 6459, 
`SpellVisualID_2` = 0, 
`SpellIconID` = 1197, 
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Suppression Aura', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Movement speed reduced by $s1%.
...
`AuraDescription_Lang_enUS` = 'Movement speed reduced by $s1%.
Time between attacks increased by $s2%.
Casting speed reduced by $s3%.', 
