INSERT INTO `spell_dbc` SET `ID` = 22667, 
`Category` = 0, 
`DispelType` = 0, 
`Mechanic` = 0, 
`Attributes` = 0, 
...
`Description_Lang_enUS` = 'Uses the guile of Deathwing to charm the target, increasing its damage by $s2% and attacking Nefarian\\s enemies for $d.', 
...
`AuraDescription_Lang_enUS` = 'Charmed!', 
