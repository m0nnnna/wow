INSERT INTO `spell_dbc` SET `ID` = 23153, 
`Category` = 0, 
`DispelType` = 1, 
`Mechanic` = 0, 
`Attributes` = 2048, 
...
`EffectBasePoints_2` = -21, 
`EffectBasePoints_3` = -31, 
`EffectMechanic_1` = 0, 
`EffectMechanic_2` = 0, 
`EffectMechanic_3` = 0, 
`ImplicitTargetA_1` = 6, 
...
`Description_Lang_enUS` = 'Burns $23393s1 mana every $t1 sec.
...
`AuraDescription_Lang_enUS` = 'Burns $23393s1 mana every $t1 sec.
Casting speed reduced by $s2%.
Movement speed reduced by $s3%.', 
