INSERT INTO `spell_dbc` SET `ID` = 23310, 
`Category` = 0, 
`DispelType` = 0, 
`Mechanic` = 0, 
`Attributes` = 0, 
...
`SpellVisualID_1` = 7039, 
`SpellVisualID_2` = 0, 
`SpellIconID` = 1481, 
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Time Lapse', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Frozen in time!
...
`AuraDescription_Lang_enUS` = 'Frozen in time!
Health reduced by $s2%.', 
