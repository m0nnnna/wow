INSERT INTO `spell_dbc` SET `ID` = 23970, 
`Category` = 0, 
`DispelType` = 0, 
`Mechanic` = 0, 
`Attributes` = 0, 
...
`SpellVisualID_1` = 7139, 
`SpellVisualID_2` = 0, 
`SpellIconID` = 45, 
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Throw Liquid Fire', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Deals $s1 Fire damage to nearby enemies.', 
