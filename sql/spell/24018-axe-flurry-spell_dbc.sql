INSERT INTO `spell_dbc` SET `ID` = 24018, 
`Category` = 0, 
`DispelType` = 0, 
`Mechanic` = 0, 
`Attributes` = 0, 
...
`EffectMiscValue_1` = 0, 
`EffectMiscValue_2` = 0, 
`EffectMiscValue_3` = 12, 
`EffectMiscValueB_1` = 0, 
`EffectMiscValueB_2` = 0, 
`EffectMiscValueB_3` = 0, 
`EffectTriggerSpell_1` = 24019, 
`EffectTriggerSpell_2` = 24071, 
...
`SpellVisualID_1` = 7163, 
`SpellVisualID_2` = 0, 
`SpellIconID` = 1474, 
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Axe Flurry', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Attacks nearby enemies in a flurry of axes that lasts $d.', 
...
`AuraDescription_Lang_enUS` = 'Attacking all nearby enemies.', 
