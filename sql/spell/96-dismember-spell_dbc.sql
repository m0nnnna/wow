INSERT INTO `spell_dbc` SET `ID` = 96, 
`Category` = 0, 
`DispelType` = 0, 
`Mechanic` = 0, 
`Attributes` = 328720, 
...
`Description_Lang_enUS` = 'Increases the damage of your next melee swing by $s1 and causes the target to bleed', 
...
`AuraDescription_Lang_enUS` = 'Bleeding for $s2 damage every $t2 seconds', 
