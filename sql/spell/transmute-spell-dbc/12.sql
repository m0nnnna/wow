INSERT INTO `spell_dbc` SET `ID` = 53771, 
`Category` = 310, 
`DispelType` = 0, 
`Mechanic` = 28, 
...
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Transmute: Eternal Life to Shadow', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Transmutes Eternal Life to Shadow.  You have a chance to discover other transmutations when successful.', 
