INSERT INTO `spell_dbc` SET `ID` = 11480, 
`Category` = 310, 
`DispelType` = 0, 
`Mechanic` = 0, 
...
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Transmute: Mithril to Truesilver', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Turns a bar of Mithril into a bar of Truesilver.', 
