INSERT INTO `spell_dbc` SET `ID` = 53779, 
`Category` = 310, 
`DispelType` = 0, 
`Mechanic` = 28, 
`Attributes` = 671088672, 
`AttributesEx` = 1024, 
`AttributesEx2` = 0, 
`AttributesEx3` = 0, 
`AttributesEx4` = 0, 
...
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Transmute: Eternal Shadow to Earth', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Transmutes Eternal Shadow to Earth.  You have a chance to discover other transmutations when successful.', 
