INSERT INTO `spell_dbc` SET `ID` = 17564, 
`Category` = 310, 
`DispelType` = 0, 
`Mechanic` = 0, 
...
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Transmute: Water to Undeath', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Taints Essence of Water turning it into Undeath.', 
