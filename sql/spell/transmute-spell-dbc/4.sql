INSERT INTO `spell_dbc` SET `ID` = 17565, 
`Category` = 310, 
`DispelType` = 0, 
`Mechanic` = 0, 
...
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Transmute: Life to Earth', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Transmutes Living Essence into Earth.', 
