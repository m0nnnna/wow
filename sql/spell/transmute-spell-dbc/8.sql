INSERT INTO `spell_dbc` SET `ID` = 53781, 
`Category` = 310, 
`DispelType` = 0, 
`Mechanic` = 28, 
...
`ActiveIconID` = 0, 
`SpellPriority` = 0, 
`Name_Lang_enUS` = 'Transmute: Eternal Earth to Air', 
`Name_Lang_deDE` = '', 
`Name_Lang_frFR` = '', 
`Name_Lang_zhCN` = '', 
`Name_Lang_esES` = '', 
...
`Description_Lang_enUS` = 'Transmutes Eternal Earth to Air.  You have a chance to discover other transmutations when successful.', 
