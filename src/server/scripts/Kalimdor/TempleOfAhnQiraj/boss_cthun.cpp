...
struct boss_eye_of_cthun : public BossAI
{
	...
	void DoAction(int32 action) override
	{
		if (action == ACTION_SPAWN_EYE_TENTACLES)
		{
			...
			// NEW CODE: Limit eye tentacles for solo play
			scheduler.Schedule(500ms, [this](TaskContext /*context*/)
			{
				std::list<Creature*> eyeTentacles;
				me->GetCreatureListWithEntryInGrid(eyeTentacles, NPC_EYE_TENTACLE, 200.f);
				
				uint8 maxEyeTentacles = 4; // Changed from 8 to 4 (adjust as needed)
				if (eyeTentacles.size() > maxEyeTentacles)
				{
					// Sort by distance and keep closest ones
					eyeTentacles.sort([this](Creature* a, Creature* b)
					{
						return me->GetDistance(a) < me->GetDistance(b);
					});
					
					auto it = eyeTentacles.begin();
					std::advance(it, maxEyeTentacles);
					
					for (auto itr = it; itr != eyeTentacles.end(); ++itr)
					{
    ...
    void ScheduleTask(bool onEngage = false)
    {
        scheduler.
			...
			.Schedule(8s, [this](TaskContext task)
			{
				...
				task.SetGroup(GROUP_BEAM_PHASE);
				task.Repeat(30s, 50s);  // Changed from Repeat() to Repeat(15s, 20s) - spawns every 15-20 seconds
			})
...
struct boss_cthun : public BossAI
{
	...
	void ScheduleTasks() override
	{
		scheduler.Schedule(13800ms, [this](TaskContext context)
		{
			...
			context.Repeat(20s, 25s);  // CHANGED: increased from 13800ms to give more time
		}).Schedule(30s, [this](TaskContext context)
		{
			if (Creature* eye = instance->GetCreature(DATA_EYE_OF_CTHUN))
				eye->AI()->DoAction(ACTION_SPAWN_EYE_TENTACLES);

			context.Repeat(45s, 60s);  // CHANGED: increased from 30s
		}).Schedule(8s, [this](TaskContext context)
		{
			...
			context.Repeat(90s, 120s);  // CHANGED: increased from 1min to 90-120s
