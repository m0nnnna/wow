...
struct boss_ouro : public BossAI
{
	...
	void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType, SpellSchoolMask) override
	{
		if (me->HealthBelowPctDamaged(20, damage) && !_enraged)
		{
			...
			scheduler.Schedule(3s, [this](TaskContext context)  // Changed initial delay
				{
					if (!IsPlayerWithinMeleeRange())
						DoSpellAttackToRandomTargetIfReady(SPELL_BOULDER);

					context.Repeat(3s, 5s);  // Changed to repeat every 3-5 seconds instead of 1s
				})
...
struct npc_dirt_mound : ScriptedAI
{
    ...
    void Reset() override
    {
       ...
       // COMMENTED OUT FOR SOLO PLAY - removes damage ticks 
