...
enum HitCounter
{
    HITCOUNTER_SLOW             = 40,   // Changed from 100
    HITCOUNTER_SLOW_MORE        = 50,   // Changed from 150
    HITCOUNTER_FREEZE           = 80,   // Changed from 200

    HITCOUNTER_CRACK            = 20,    // Changed from 50
    HITCOUNTER_SHATTER          = 30,   // Changed from 100
    HITCOUNTER_EXPLODE          = 40,   // Changed from 150
};
...
struct boss_viscidus : public BossAI
{
	...
	void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType effType, SpellSchoolMask spellSchoolMask) override
	{
		...
		{
			// For solo play: count ALL direct damage as frost damage
	...
	void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
	{
		if (spellInfo->Id == SPELL_REJOIN_VISCIDUS)
		{
			...
			return; // ADDED: exit early for this spell
		}

		// CHANGED FOR SOLO: Simplified to count all spell hits during frost phase
...
class spell_explode_trigger : public SpellScript
{
	...
	void HandleOnHit()
	{
		...
		// Limit globs for solo play
		globsToSpawn = std::min(globsToSpawn, (uint8)5);  // Max 5 globs instead of 20
